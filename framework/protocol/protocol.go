...
const (
	...
	Handshake    PackageType = 0x01 // Handshake 表示握手：请求（客户端） <====> 握手响应（服务器）
	HandshakeAck PackageType = 0x02 // HandshakeAck 表示从客户端到服务器的握手确认
	Heartbeat    PackageType = 0x03 // Heartbeat 表示心跳
	Data         PackageType = 0x04 // Data 表示一个普通的数据包
	Kick         PackageType = 0x05 // Kick 表示一个踢出包
)
