...
type MessageData struct {
	Cuopai      bool   `json:"cuopai"`      // 是否搓牌，true 表示搓牌，false 表示不搓牌
	Score       int    `json:"score"`       // 下分的分数
	Type        int    `json:"type"`        // 1 跟注 2 加注
	ChairID     int    `json:"chairID"`     // 玩家在房间中的座位号
	Msg         string `json:"msg"`         // 聊天的内容
	RecipientID int    `json:"recipientID"` // 收消息的ID
}
type GameStatus int
...
// GameBankerPushData {"type":414,"data":{"bankerChairID":0},"pushRouter":"GameMessagePush"}
// 庄家推送
...
// GameBureauPushData {"type":411,"data":{"curBureau":6},"pushRouter":"GameMessagePush"}
// 局数推送， 进行到第几局
...
// GameSendCardsPushData 发牌推送， 把每个玩家的手牌发给对应的玩家
...
// GamePourScorePushData 下分推送
func GamePourScorePushData(chairID, score, chairScore, scores, t int) any {
	return map[string]any{
		"type": GamePourScorePush,
		"data": map[string]any{
			"chairID":    chairID,    // 座位号
			"score":      score,      // 玩家当前拥有分数
			"chairScore": chairScore, // 当前座位所下的分数
			"scores":     scores,     // 所有用户下的总分数
			"type":       t,          // 类型
		},
		"pushRouter": "GameMessagePush",
	}
...
// GameLookPushData {"type":403,"data":{"chairID":1,"cards":[60,2,44],"cuopai":false},"pushRouter":"GameMessagePush"}
// 把看牌的数据推送给操作者
